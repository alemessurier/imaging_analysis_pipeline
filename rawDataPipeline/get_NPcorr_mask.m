...
if length(ROI_path)==1
    ...
    try
        ...
        assert(length(cellNames)==size(npMasks_new,3),...%changed from npMasks_new 20180814
        ...
        np_ROIcorr=calc_npMaskCorr_byPix(curr_im,npMasks_new,cellNames,corrThreshold,rawTimeSeries.(fns{1}));
...
function [neuropil_mask_ROIcorr]=calc_npMaskCorr_byPix(curr_im,NPmasks_new,cellNames,corrThreshold,rawTimeSeries)

cn_inds=1:length(cellNames);
neuropil_mask_ROIcorr=zeros(512,512,length(cellNames));

parfor i=1:size(NPmasks_new,3)
    thisNPMask=NPmasks_new(:,:,i); % pick one npMask at a time
