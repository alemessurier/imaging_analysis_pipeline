function voronoiPlots_L4_20180531(paths_EN,paths_NH,type,npSub)
...
for i=1:numel(ROIs_NH)
    ...
    ROIs_CRow_NH{i}=ROIs_NH{i}(inds_CRow);
       ...
       for K=1:length(paths_EN)
            [ROI_pos_plot_EN{K},responses_plot_EN{K}] = colorCodeByWhiskResponse_norm( paths_EN{K},type,npSub,ROIs_CRow_EN{K} );
        ...
        for K=1:length(paths_NH)
            [ROI_pos_plot_NH{K},responses_plot_NH{K}] = colorCodeByWhiskResponse_norm( paths_NH{K},type,npSub,ROIs_CRow_NH{K} );
        end
