function voronoi_decode_20171219(paths_EN,paths_NH,cells)
...
switch cells
    ...
    case 'CW'
        % CW-tuned ROIs
        for K=1:length(paths_EN)
           [ ROI_pos_plot_EN{K},perfByWhisk_EN{K},PbyWshuff_EN{K} ] = decoder_PandPos( paths_EN{K},'CW' );
        end
        
        
        % CW-tuned ROIs
        for K=1:length(paths_NH)
           [ ROI_pos_plot_NH{K},perfByWhisk_NH{K},PbyWshuff_NH{K} ] = decoder_PandPos( paths_NH{K},'CW' );
        end
    case 'SW'
       % SW-tuned ROIs
        for K=1:length(paths_EN)
           [ ROI_pos_plot_EN{K},perfByWhisk_EN{K},PbyWshuff_EN{K} ] = decoder_PandPos( paths_EN{K},'SW' );
        end
        
        
        % SW-tuned ROIs
        for K=1:length(paths_NH)
           [ ROI_pos_plot_NH{K},perfByWhisk_NH{K},PbyWshuff_NH{K} ] = decoder_PandPos( paths_NH{K},'SW' );
        end
end
